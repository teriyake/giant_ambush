...
public class NetworkUIHandlerRelay : MonoBehaviour
{
        ...
        if (!_gameManagerSpawned)
        {
            if (m_gameManagerPrefab != null)
            {
                ...
                if (networkObject != null)
                {
                    ...
                    {
                        GameManager.Instance.VRClientId.Value = NetworkManager
                            .Singleton
                            .LocalClientId;
                        Debug.Log(
                    ...
                    {
                        Debug.LogError(
                            "NetworkUIHandlerRelay: GameManager.Instance is null AFTER spawning in OnServerStarted!"
                        );
                        SetStatus("Error: GameManager not available after spawn.");
                    }
                }
                else
                {
    ...
    void OnClientConnected(ulong clientId)
    {
        ...
        {
            Debug.LogError(
                $"NetworkUIHandlerRelay: GameManager.Instance is null in OnClientConnected for client {clientId}!"
            );
        }
    }
