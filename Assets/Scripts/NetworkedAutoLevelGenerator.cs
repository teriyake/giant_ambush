...
public class NetworkedAutoLevelGenerator : NetworkBehaviour
{
    ...
    [SerializeField]
    private BlocksRepo m_blocksRepo;

    [SerializeField]
    private float m_blockSize = 1.0f;

    [SerializeField]
    private int m_levelHeight = 5;
    ...
    public event Action OnGenerationCompleteServer;
    ...
    void Awake()
    {
        ...
        {
            Debug.LogError("NALG: BlocksRepo null", this);
            enabled = false;
            return;
        }
        if (m_meshRoot == null)
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsServer)
        {
            ...
            {
                Debug.LogError(
                    $"NALG ({NetworkObjectId}): Could not find GameManager Instance to register with!"
                );
            }
        }
    }
    ...
    private void InitializeAutoLevelCommon()
    {
        ...
        Debug.Log(
            $"NALG ({NetworkObjectId} - IsServer: {IsServer}): Initializing AutoLevel Common (Runtime Repo)."
        );
    ...
    public void GenerateLevel(Vector2 realWorldSize)
    {
        ...
        {
            Debug.LogWarning(
                $"NALG ({NetworkObjectId}): GenerateLevel called on non-server.",
                this
            );
            return;
        }
        ...
        if (runtimeRepo == null)
        {
            Debug.LogError($"NALG (Server {NetworkObjectId}): RuntimeRepo null", this);
            return;
        }
        ...
        if (iterations > 0)
        {
            ...
            if (blockData == null)
            {
                Debug.LogError("NALG (Server): Failed to serialize LevelData!");
                return;
            }

            Debug.Log(
                $"NALG (Server {NetworkObjectId}): Sending BuildLevelClientRpc with Bounds={m_generationBounds} and {blockData.Length} block data points."
            );
            ...
            Debug.Log(
                $"NALG (Server {NetworkObjectId}): Invoking OnGenerationCompleteServer event directly."
            );
            // OnGenerationCompleteServer?.Invoke(); // Invoke directly causes timing issues
            StartCoroutine(InvokeCompletionEventAfterFrame()); // Invoke after frame allows clients to potentially receive RPC first
        }
        else
        {
    ...
    private IEnumerator InvokeCompletionEventAfterFrame()
    {
        yield return new WaitForEndOfFrame();

        Debug.Log(
            $"NALG (Server {NetworkObjectId}): Invoking OnGenerationCompleteServer event (End Of Frame)."
        );
        OnGenerationCompleteServer?.Invoke();
    }
    ...
    private void RebuildMeshLocally(LevelData localLevelData)
    {
        ...
        {
            Debug.LogError(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Cannot rebuild mesh, RuntimeRepo or LevelData is null."
            );
            return;
        }
        ...
        {
            Debug.Log(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Destroying previous AutoLevel 'root' GameObject before rebuild."
            );
        ...
        {
            Debug.Log(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Disposing previous MeshBuilder."
            );
            meshBuilder.Dispose();
            meshBuilder = null;
        }
        ...
        Debug.Log(
            $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Creating new LevelMeshBuilder."
        );
        meshBuilder = new LevelMeshBuilder(localLevelData, runtimeRepo);

        Debug.Log(
            $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Rebuilding mesh locally for bounds {localLevelData.bounds}..."
        );
        ...
        {
            Debug.Log(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Found AutoLevel 'root' GO '{autoLevelRootGO.name}' at scene root. Repositioning and reparenting..."
            );

            autoLevelRootGO.transform.SetPositionAndRotation(
                transform.position,
                transform.rotation
            );
            ...
            Debug.Log(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Positioned '{autoLevelRootGO.name}' at world {autoLevelRootGO.transform.position}, Parented under {transform.name}."
            );
        }
        else if (autoLevelRootGO != null && autoLevelRootGO.transform.parent != null)
        {
            Debug.LogWarning(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Found a 'root' GO, but it was already parented to {autoLevelRootGO.transform.parent.name}. Assuming it's not the one just created by AutoLevel or was handled differently.",
                autoLevelRootGO
            );
        }
        else
        {
            Debug.LogError(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Failed to find AutoLevel's generated 'root' GameObject at scene root after Rebuild! Level geometry will be at the wrong place."
            );
        }

        // ApplyMeshRootOffset();
        Debug.Log(
            $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Local mesh rebuild complete. Mesh root at {m_meshRoot.localPosition}"
        );
    }

    [ClientRpc]
    private void BuildLevelClientRpc(
        Vector3Int boundsPosition,
        Vector3Int boundsSize,
        int[] receivedBlockData
    )
    {
        Debug.Log(
            $"NALG (Client {NetworkManager.Singleton.LocalClientId}): Received BuildLevelClientRpc."
        );
        ...
        {
            Debug.LogError(
                $"NALG (Client {NetworkManager.Singleton.LocalClientId}): RuntimeRepo null",
                this
            );
            return;
        }
        ...
        {
            Debug.LogError(
                $"NALG (Client {NetworkManager.Singleton.LocalClientId}): Failed to deserialize LevelData!"
            );
            return;
        }
        Debug.Log(
            $"NALG (Client {NetworkManager.Singleton.LocalClientId}): Successfully deserialized LevelData for bounds {m_generationBounds}."
        );
    ...
    private void ApplyMeshRootOffset()
    {
        ...
        {
            Debug.LogWarning(
                $"NALG ({NetworkObjectId} - IsServer:{IsServer}): Cannot apply offset, generation bounds size is zero."
            );
            return;
        }
        ...
        if (m_meshRoot != null)
        {
            ...
            Debug.Log(
                $"NALG ({NetworkObjectId} - IsServer: {IsServer}): Applied local offset {centerOffset} to mesh root '{m_meshRoot.name}'."
            );
        }
    }
    ...
    private bool DeserializeLevelData(LevelData data, int[] serializedData)
    {
        ...
        {
            Debug.LogError(
                "NALG Deserialize: Input LevelData, its Blocks array, or serializedData is null."
            );
            return false;
        }
        ...
        {
            Debug.LogError(
                $"NALG Deserialize: Data size mismatch! Expected {expectedTotal}, got {serializedData.Length}"
            );
            return false;
        }
        if (expectedTotal == 0)
    ...
    {
        StopAllCoroutines();
        OnGenerationCompleteServer = null;
    ...
    {
        string role = IsServer
            ? "Server"
            : $"Client {NetworkManager.Singleton?.LocalClientId ?? 0}";
