...
public class ARLevelSetupNALG : NetworkBehaviour
{
    ...
    private void RequestLevelSpawnServerRpc(Vector3 position, Quaternion rotation, Vector2 size, ServerRpcParams rpcParams = default)
    {
        ...
        Vector3Int estimatedLevelSizeBlocks = new Vector3Int(Mathf.CeilToInt(size.x / blockSize) * 5, levelHeight, Mathf.CeilToInt(size.y / blockSize) * 5);
        ...
        */
        ...
        Debug.Log($"Server: Calling GenerateLevel() with size {size} * 5 on {roomSpawnerObject.name} ({networkObject.NetworkObjectId})");
        autoLevelGenerator.GenerateLevel(size * 5);
